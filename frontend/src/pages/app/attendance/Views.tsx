...
import { iso, orgHeatmap, personalStatus, statusMeta, today, type DayStatus } from './data'

const LEGEND = Object.entries(statusMeta) as [keyof typeof statusMeta, (typeof statusMeta)[keyof typeof statusMeta]][]

export function MonthCalendar({ seed, holidays }: { seed: string; holidays: Set<string> }) {
  ...
  const days = useMemo(() => {
    ...
    return eachDayOfInterval({ start, end }).map((d) => {
      const date = iso(d)
      return { d, date, inMonth: isSameMonth(d, month), status: personalStatus(seed, date, holidays) }
    })
  }, [month, seed, holidays])
...
export function AttendanceHeatmap({ holidays, seed }: { holidays: Set<string>; seed: string }) {
  const weeks = useMemo(() => orgHeatmap(16, holidays, seed), [holidays, seed])
  ...
  return (
    <div>
          ...
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((day, di) => (
            <div key={day} className="flex items-center gap-1">
              ...
              {weeks.map((w, wi) => {
                const c = w[di]!
                return (
                  <Tip key={c.date} label={c.pct === null ? `${formatDate(c.date)} · upcoming` : `${formatDate(c.date)} · ${c.pct}% present`}>
                    <motion.div
                      initial={{ opacity: 0, scale: 0.6 }}
                      animate={{ opacity: 1, scale: 1 }}
                      transition={{ delay: wi * 0.015 + di * 0.01 }}
                      className={cn('size-4 shrink-0 rounded-[4px] sm:size-5', c.date === TODAY && 'ring-2 ring-foreground/60')}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
        <span>16-week average: <span className="font-semibold text-foreground tabular">{avg}%</span> present</span>
